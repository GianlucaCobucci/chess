...
function dragDrop(event) {
    ...
    if (correctGo) {
        ...
        //poi controlla questo
        if (taken && !takenByOpponent) {
            infoDisplay.textContent = "Non puoi fare questa mossa"
...
function checkForWin() {
    ...
    if (!kings.some(king => king.firstChild.classList.contains("white"))) {
        infoDisplay.innerHTML = "Il giocatore nero ha vinto"
        const allSquares = document.querySelectorAll(".square")
        allSquares.forEach(square => square.firstChild?.setAttribute("draggable", false))
    }
    if (!kings.some(king => king.firstChild.classList.contains("black"))) {
        infoDisplay.innerHTML = "Il giocatore bianco ha vinto"
        const allSquares = document.querySelectorAll(".square")
        allSquares.forEach(square => square.firstChild?.setAttribute("draggable", false))
    }
}
...
function isOccupied(squareId) {
    return document.querySelector(`[square-id="${squareId}"]`).firstChild !== null;
}
...
function checkIfValid(target) {
    ...
    switch (piece) {
        case "pawn":
            ...
            break;
        case "knight":
            const knightMoves = [width * 2 + 1, width * 2 - 1, width + 2, width - 2, -width * 2 + 1, -width * 2 - 1, -width + 2, -width - 2];
